                                              ...
                                              v_U8_t  macCount )
{
   ...
   {
      VOS_TRACE(VOS_MODULE_ID_VOSS, VOS_TRACE_LEVEL_ERROR,
                "%s: Not able to get VosContext", __func__);
      return VOS_STATUS_E_FAILURE;
   }
   ...
   {
      int macLoop = 0;
      /* We have already read MAC addresses when this function was called from
         vos_nv_open. Avoid reparsing and fill from the global NV structure */
      if (((VosContextType*)(pVosContext))->nvVersion == E_NV_V2)
      {
         vos_mem_copy(pMacAddress,
                      gnvEFSTableV2->halnvV2.fields.macAddr,
                      VOS_MAC_ADDRESS_LEN);
         vos_mem_copy(pMacAddress + 1 * VOS_MAC_ADDRESS_LEN,
                      gnvEFSTableV2->halnvV2.fields.macAddr2,
                      VOS_MAC_ADDRESS_LEN);
         vos_mem_copy(pMacAddress + 2 * VOS_MAC_ADDRESS_LEN,
                      gnvEFSTableV2->halnvV2.fields.macAddr3,
                      VOS_MAC_ADDRESS_LEN);
         vos_mem_copy(pMacAddress + 3 * VOS_MAC_ADDRESS_LEN,
                      gnvEFSTableV2->halnvV2.fields.macAddr4,
                      VOS_MAC_ADDRESS_LEN);
      }
      else if(((VosContextType*)(pVosContext))->nvVersion == E_NV_V3)
      {
         vos_mem_copy(pMacAddress,
                      gnvEFSTable->halnv.fields.macAddr, VOS_MAC_ADDRESS_LEN);
         vos_mem_copy(pMacAddress + 1 * VOS_MAC_ADDRESS_LEN,
                      gnvEFSTable->halnv.fields.macAddr2, VOS_MAC_ADDRESS_LEN);
         vos_mem_copy(pMacAddress + 2 * VOS_MAC_ADDRESS_LEN,
                      gnvEFSTable->halnv.fields.macAddr3, VOS_MAC_ADDRESS_LEN);
         vos_mem_copy(pMacAddress + 3 * VOS_MAC_ADDRESS_LEN,
                      gnvEFSTable->halnv.fields.macAddr4, VOS_MAC_ADDRESS_LEN);
      }
      else
      {
         VOS_TRACE(VOS_MODULE_ID_VOSS, VOS_TRACE_LEVEL_ERROR,
      ...
      {
         VOS_TRACE(VOS_MODULE_ID_VOSS, VOS_TRACE_LEVEL_ERROR,
                   "%s: pMacAddress[%d]=" MAC_ADDRESS_STR, __func__, macLoop,
                   MAC_ADDR_ARRAY(pMacAddress + macLoop * VOS_MAC_ADDRESS_LEN));
      }
      return VOS_STATUS_SUCCESS;
   }
   ...
   else
   {
      ...
      else
      {
         ...
         {
            VOS_TRACE(VOS_MODULE_ID_VOSS, VOS_TRACE_LEVEL_ERROR,
                      "%s: " WIFI_MAC_BOOTARG " not present in bootargs",
                      __func__);
         }
         else
         {
            ...
            /* Handle inter MAC separator if any */
            if (*mac_idx == ',' || *mac_idx == '-')
               mac_idx++;
            else
               VOS_TRACE(VOS_MODULE_ID_VOSS, VOS_TRACE_LEVEL_ERROR,
                         "No inter MAC separator used");

            memcpy(macStr2, mac_idx, MACSTRLEN);
            VOS_TRACE(VOS_MODULE_ID_VOSS, VOS_TRACE_LEVEL_ERROR,
                      "%s: MAC1 from bootparams=%s\n", __func__, macStr1);
            VOS_TRACE(VOS_MODULE_ID_VOSS, VOS_TRACE_LEVEL_ERROR,
                      "%s: MAC2 from bootparams=%s\n", __func__, macStr2);
            ...
            VOS_TRACE(VOS_MODULE_ID_VOSS, VOS_TRACE_LEVEL_DEBUG,
                      "%s: Mac1 Addr: "  MAC_ADDRESS_STR, __func__,
                      MAC_ADDR_ARRAY(mac1));
            VOS_TRACE(VOS_MODULE_ID_VOSS, VOS_TRACE_LEVEL_DEBUG,
                      "%s: Mac2 Addr: "  MAC_ADDRESS_STR, __func__,
                      MAC_ADDR_ARRAY(mac2));
            VOS_TRACE(VOS_MODULE_ID_VOSS, VOS_TRACE_LEVEL_DEBUG,
                      "%s: Mac3 Addr: "  MAC_ADDRESS_STR, __func__,
                      MAC_ADDR_ARRAY(mac3));
            VOS_TRACE(VOS_MODULE_ID_VOSS, VOS_TRACE_LEVEL_DEBUG,
                      "%s: Mac4 Addr: "  MAC_ADDRESS_STR, __func__,
                      MAC_ADDR_ARRAY(mac4));
         }
      }
   }
   ...
   if (VOS_STATUS_SUCCESS == status)
   {
      ...
      if (((VosContextType*)(pVosContext))->nvVersion == E_NV_V2)
      {
         vos_mem_copy(gnvEFSTableV2->halnvV2.fields.macAddr, mac1,
                      VOS_MAC_ADDRESS_LEN);
         vos_mem_copy(gnvEFSTableV2->halnvV2.fields.macAddr2, mac2,
                      VOS_MAC_ADDRESS_LEN);
         vos_mem_copy(gnvEFSTableV2->halnvV2.fields.macAddr3, mac3,
                      VOS_MAC_ADDRESS_LEN);
         vos_mem_copy(gnvEFSTableV2->halnvV2.fields.macAddr4, mac4,
                      VOS_MAC_ADDRESS_LEN);
         VOS_TRACE(VOS_MODULE_ID_VOSS, VOS_TRACE_LEVEL_ERROR,
                   "%s MAC address in global V2"
                   MAC_ADDRESS_STR MAC_ADDRESS_STR MAC_ADDRESS_STR MAC_ADDRESS_STR,
                   __func__,
                   MAC_ADDR_ARRAY(gnvEFSTableV2->halnvV2.fields.macAddr),
                   MAC_ADDR_ARRAY(gnvEFSTableV2->halnvV2.fields.macAddr2),
                   MAC_ADDR_ARRAY(gnvEFSTableV2->halnvV2.fields.macAddr3),
                   MAC_ADDR_ARRAY(gnvEFSTableV2->halnvV2.fields.macAddr4));
      }
      else if(((VosContextType*)(pVosContext))->nvVersion == E_NV_V3)
      {
         vos_mem_copy(gnvEFSTable->halnv.fields.macAddr, mac1,
                      VOS_MAC_ADDRESS_LEN);
         vos_mem_copy(gnvEFSTable->halnv.fields.macAddr2, mac2,
                      VOS_MAC_ADDRESS_LEN);
         vos_mem_copy(gnvEFSTable->halnv.fields.macAddr3, mac3,
                      VOS_MAC_ADDRESS_LEN);
         vos_mem_copy(gnvEFSTable->halnv.fields.macAddr4, mac4,
                      VOS_MAC_ADDRESS_LEN);
         VOS_TRACE(VOS_MODULE_ID_VOSS, VOS_TRACE_LEVEL_ERROR,
                   "%s MAC address in global V3"
                   MAC_ADDRESS_STR MAC_ADDRESS_STR MAC_ADDRESS_STR MAC_ADDRESS_STR,
                   __func__,
                   MAC_ADDR_ARRAY(gnvEFSTable->halnv.fields.macAddr),
                   MAC_ADDR_ARRAY(gnvEFSTable->halnv.fields.macAddr2),
                   MAC_ADDR_ARRAY(gnvEFSTable->halnv.fields.macAddr3),
                   MAC_ADDR_ARRAY(gnvEFSTable->halnv.fields.macAddr4));
      }
      else
      {
         VOS_TRACE(VOS_MODULE_ID_VOSS, VOS_TRACE_LEVEL_ERROR,
